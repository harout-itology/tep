<?php
    ...
    function addressSearch($db,$defaultfields,$colsfields){
      ...
      $countresults=array();
      if($fcity=='' && $fcounty==''){
         $citycountquery="SELECT city as name, count( * ) AS counter FROM db_st_towerco_new WHERE state = '$fstate' $towerowerand GROUP BY city";
         $countycountquery="SELECT county  as name, count( * ) AS counter FROM db_st_towerco_new WHERE state = '$fstate' $towerowerand GROUP BY county";
         $sccountquery="SELECT StructureClassification  as name, count( * ) AS counter FROM db_st_towerco_new WHERE state = '$fstate' $towerowerand GROUP BY StructureClassification";
         $towerownercountquery="SELECT TowerOwner  as name, count( * ) AS counter FROM db_st_towerco_new WHERE state = '$fstate' $towerowerand  GROUP BY TowerOwner";
         $statecountquery="SELECT state  as name, count( * ) AS counter FROM db_st_towerco_new WHERE state = '$fstate' $towerowerand  GROUP BY state";
